//Árvores binárias de busca - caminho com maior valor de soma entre os nós
#include <stdio.h>
#include <string.h>
#include <math.h>
#include <stdlib.h>
...
int soma_caminho(Node* tree, int soma){
    if(tree == NULL)
        return soma;
    
    soma += tree->item;
    
    int soma_esq = soma_caminho(tree->left, soma);
    int soma_dir = soma_caminho(tree->right, soma);
...
int main(){
    ...
    for(i = 0; i < n; i++){
        scanf("%d", &num);
        tree = inserir(num, tree);
    }
    
    int soma = soma_caminho(tree, 0);
