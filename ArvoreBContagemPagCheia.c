...
void conta_paginas_cheias(NodeB *tree, int *cont){
    int i;
    if(tree!=NULL){
        *cont+=(tree->nro_chaves==N-1);
        if(tree->filhos[0]!=NULL){
            if(tree->filhos[0]->eh_no_folha){
                 for(i=0; i<=tree->nro_chaves; i++)
                      *cont+=(tree->filhos[0]->nro_chaves==N-1);
            }
            for(i=0; i<=tree->nro_chaves; i++)
                conta_paginas_cheias(tree->filhos[i], cont);  
        }
    }
}
int main(){
    ...
    conta_paginas_cheias(tree, &cont);
    printf("%d", cont);
