...
int cont_nos_pretos(NodeVP *tree, int cont){
    NodeVP *aux=tree;
    if(aux != NULL){
        if(aux->color==1 && aux->key%2!=0)
            cont++;
        cont=cont_nos_pretos(aux->right, cont);
        cont=cont_nos_pretos(aux->left, cont);
    }
    return cont;
}
...
int main(){
    ...
    printf("%d", cont_nos_pretos(tree, 0));
